        ...
        <div class="group overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
            ...
            <div class="p-5">
                <div class="mb-3 flex items-center justify-between gap-3">
                    <a href="{{ route('projects.show', $project) }}" class="font-semibold group-hover:text-indigo-600">{{ $project->title }}</a>
                    <div class="flex items-center gap-2">
                        <form method="POST" action="{{ route('projects.favorite', $project) }}">
                            @csrf
                            <button class="text-lg text-amber-400 transition hover:scale-110" aria-label="Toggle favorite">{{ in_array($project->id, $favoriteProjectIds ?? [], true) ? '★' : '☆' }}</button>
                        </form>
