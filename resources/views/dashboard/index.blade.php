...
@section('content')
    @if (auth()->user()->isAdmin() || auth()->user()->isProjectManager())
        <section class="mb-5 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm sm:p-5">
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        @foreach ([
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition hover:shadow-md sm:p-5">
                ...
                @forelse ($recentProjects as $project)
                    <a href="{{ route('projects.show', $project) }}" class="block rounded-2xl border border-gray-100 p-4 transition hover:border-indigo-200 hover:bg-indigo-50/30">
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <span class="shrink-0 rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">{{ $project->tasks_count }} tasks</span>
                        </div>
                    </a>
                @empty
            ...
            <div class="space-y-3">
                @forelse ($upcomingTasks as $task)
                    <a href="{{ route('projects.show', $task->project) }}" class="flex items-center justify-between gap-4 rounded-2xl border border-gray-100 p-4 transition hover:bg-gray-50">
                        ...
                        <span class="shrink-0 rounded-full bg-rose-50 px-3 py-1 text-xs font-semibold text-rose-700">{{ $task->due_date?->format('M d') }}</span>
                    </a>
                @empty
